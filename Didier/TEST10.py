...
class Agenda:
    ...
    def CargarContactos(self):
        try:
            ...
            print("ERROR: No existe el archivo de la agenda")
        else:
            contactos = fichero.readlines()
            fichero.close()
            if len(contactos) > 0:
                for contacto in contactos:
                    datos = contactos.split("#")
                    if len(datos) == 11:
...
def ProcesoCrearContacto(agenda):
    ...
    nuevocontacto.SetCodigoPostal(input(">Introduce el código postal del contacto:"))
...
3) Volver""")
    ...
    while not finbuscar:
        ...
        if opcbuscar == 1:
            ...
            agenda.BorrarContactoPorTelefono(input(">Introduce el teléfono del contacto a borrar:"))
            finbuscar = True
        elif opcbuscar == 3:
            finbuscar = True
